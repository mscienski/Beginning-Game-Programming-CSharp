...
namespace ProgrammingAssignment6
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            deck = new Deck(Content, graphics.PreferredBackBufferWidth / 2, graphics.PreferredBackBufferHeight / 3);
            deck.Shuffle();
            ...
            dealerHand[0].X = graphics.PreferredBackBufferWidth - HORIZONTAL_CARD_OFFSET;
            ...
            playerHand[1].Y = playerHand[0].Y + VERTICAL_CARD_SPACING;

            // second dealer card
            dealerHand.Add(deck.TakeTopCard());
            dealerHand[1].FlipOver();
            dealerHand[1].X = graphics.PreferredBackBufferWidth - HORIZONTAL_CARD_OFFSET;
            dealerHand[1].Y = dealerHand[0].Y + VERTICAL_CARD_SPACING;
            ...
            menuButtons.Add(new MenuButton(Content.Load<Texture2D>("hitbutton"), new Vector2(graphics.PreferredBackBufferWidth/2, TOP_MENU_BUTTON_OFFSET), GameState.PlayerHitting));

            // create stand button and add to list
            menuButtons.Add(new MenuButton(Content.Load<Texture2D>("standbutton"), new Vector2(graphics.PreferredBackBufferWidth/2, TOP_MENU_BUTTON_OFFSET + VERTICAL_MENU_BUTTON_SPACING), GameState.WaitingForDealer));
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            switch (currentState)
            {
                case GameState.PlayerHitting:
                    ...
                    playerHand[playerHand.Count - 1].Y = playerHand[playerHand.Count - 2].Y + VERTICAL_CARD_SPACING;
                ...
                case GameState.CheckingHandOver:
                    if (GetBlackjackScore(playerHand) > MAX_HAND_VALUE || GetBlackjackScore(dealerHand) > MAX_HAND_VALUE)
                    {
                        ...
                        {
                            messages.Add(new Message("TIE GAME", messageFont, winnerMessageLocation));
                        }
                        else if (GetBlackjackScore(playerHand) <= MAX_HAND_VALUE)
                        {
                            messages.Add(new Message("PLAYER WINS", messageFont, winnerMessageLocation));
                        }
                        else if (GetBlackjackScore(dealerHand) <= MAX_HAND_VALUE)
                        {
                            messages.Add(new Message("DEALER WINS", messageFont, winnerMessageLocation));
                        }

                        dealerHand[0].FlipOver();
                        messages.Add(new Message(SCORE_MESSAGE_PREFIX + GetBlackjackScore(dealerHand).ToString(), messageFont, new Vector2(graphics.PreferredBackBufferWidth-HORIZONTAL_MESSAGE_OFFSET, SCORE_MESSAGE_TOP_OFFSET)));
                        
                        for (int i=menuButtons.Count-1; i>=0; i--) {
                            menuButtons.RemoveAt(i);
                        }

                        menuButtons.Add(new MenuButton(Content.Load<Texture2D>("quitbutton"), new Vector2(graphics.PreferredBackBufferWidth/2, graphics.PreferredBackBufferHeight-VERTICAL_MENU_BUTTON_SPACING), GameState.Exiting));
                ...
                case GameState.DealerHitting:
                    ...
                    dealerHand[dealerHand.Count - 1].X = graphics.PreferredBackBufferWidth - HORIZONTAL_CARD_OFFSET;
                    dealerHand[dealerHand.Count - 1].Y = dealerHand[dealerHand.Count - 2].Y + VERTICAL_CARD_SPACING;
